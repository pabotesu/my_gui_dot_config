...
if [ "${1}" == '-h' ]; then
    ...
    if [ "${1}" != '-s' ]; then
        help_and_exit "error: unknown optin ${1}"
    fi
    base_folder="${2}"
else
    base_folder="${1}"
...
file_path=${base_folder}$( date '+%Y-%m-%d_%H-%M-%S' )_screenshot.jpg
import ${params} ${file_path}
xclip -selection clipboard -target image/jpg -i < ${file_path}
notify-send "YOU HAVE A SCREENSHOT!!!" "file-path : ${file_path}"
